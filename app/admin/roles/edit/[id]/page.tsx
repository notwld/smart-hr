...
export default function EditRolePage({ params }: { params: { id: string } }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (permissionsResponse.ok) {
          ...
          setSelectedPermissions(rolePermissions.map((rp: any) => rp.permissionId));
        }
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updateRoleResponse = await fetch(`/api/admin/roles/${roleId}`, {
        method: "PUT",
        headers: {
      ...
      if (!updateRoleResponse.ok) {
        const error = await updateRoleResponse.json();
        throw new Error(error.message || "Failed to update role");
      }
      
      // Get current permissions for the role
      const currentPermissionsRes = await fetch(`/api/admin/roles/${roleId}/permissions`);
      const currentPermissions = await currentPermissionsRes.json();
      ...
      const permissionsToAdd = selectedPermissions.filter((id: string) => !currentPermissionIds.includes(id));
      const permissionsToRemove = currentPermissionIds.filter((id: string) => !selectedPermissions.includes(id));
  ...
  return (
    <PermissionGuard permissions="roles.edit" fallback={<div>You don't have permission to access this page.</div>}>
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">Modify role details and permissions</p>
            </div>
          </div>
        </div>
